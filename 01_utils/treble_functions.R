...
get_windows <- function(features,
  ...
  win <-
    split_with_overlap(features, window_size, window_size - step_size)
  ...
  if (is.null(name) == FALSE) {
    ...
    colnames(win) <-
      paste(name, "_", rownames(features)[1:ncol(win)], sep = "")
  } else {
...
run_procrustes <- function(umaps,
  ...
  for (i in 1:ncol(x)) {
    ...
    dists <- c(dists, euc_dist(umaps[[x[1, i]]][, 1:2],
                               umaps[[x[2, i]]][, 1:2]))
  ...
  if (length(series) > 1) {
    ...
    if (!is.null(n_trajectories)) {
      if (ncol(series) > n_trajectories) {
        ...
        series <-
          series[, sample(seq(1, ncol(series), 1), n_trajectories)]
        print(ncol(series))
